...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'OrderDayItem', fields ['dish', 'order']
        db.delete_unique('dinner_orderdayitem', ['dish_id', 'order_id'])

        # Deleting field 'OrderDayItem.dish'
        db.delete_column('dinner_orderdayitem', 'dish_id')

        # Adding field 'OrderDayItem.dish_day'
        db.add_column('dinner_orderdayitem', 'dish_day', self.gf('django.db.models.fields.related.ForeignKey')(default=0, to=orm['dinner.DishDay']), keep_default=False)

        # Adding unique constraint on 'OrderDayItem', fields ['dish_day', 'order']
        db.create_unique('dinner_orderdayitem', ['dish_day_id', 'order_id'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'OrderDayItem', fields ['dish_day', 'order']
        db.delete_unique('dinner_orderdayitem', ['dish_day_id', 'order_id'])

        # User chose to not deal with backwards NULL issues for 'OrderDayItem.dish'
        raise RuntimeError("Cannot reverse this migration. 'OrderDayItem.dish' and its values cannot be restored.")

        # Deleting field 'OrderDayItem.dish_day'
        db.delete_column('dinner_orderdayitem', 'dish_day_id')

        # Adding unique constraint on 'OrderDayItem', fields ['dish', 'order']
        db.create_unique('dinner_orderdayitem', ['dish_id', 'order_id'])
